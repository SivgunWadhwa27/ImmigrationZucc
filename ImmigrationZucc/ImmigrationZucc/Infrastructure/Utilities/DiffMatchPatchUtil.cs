...
namespace ImmigrationZucc.Infrastructure.Utilities {
  
    public static class DiffMatchPatchUtil
    {
        public static string GetPatchedString(string beforeStr, string afterStr)
        {
            var dmp = new diff_match_patch();
            var diffs = dmp.diff_main(beforeStr, afterStr);
            dmp.diff_cleanupSemantic(diffs);
            var patches = dmp.patch_make(diffs);
            var patchedStr = dmp.patch_toText(patches);
            return patchedStr;
        }
    }
